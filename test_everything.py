...
print('=' * 70)
print('STARTING COMPREHENSIVE 12-POINT TEST SUITE FOR ALL 10 TOOLS & 3 UI WIDGETS')
print('=' * 70)
...
print('\n[1/12] Testing MCP Handshake (initialize)...')
...
# TEST 2: Tools Listing (All 10 Tools)
print('\n[2/12] Testing Tools Listing (tools/list)...')
...
assert_test('Exactly 10 tools registered', len(tool_names) == 10, f'got {len(tool_names)}')
for et in expected_tools:
    assert_test(f'Tool registered: {et}', et in tool_names)

# TEST 3: UI Resources (All 3 HTML Widgets)
print('\n[3/12] Testing UI Resources (resources/list & read)...')
...
assert_test('Resource ui://tutor/web-sandbox exists', 'ui://tutor/web-sandbox' in res_uris)
...
print('\n[4/12] Testing Code Mission Flow (off-by-one-sum)...')
...
print('\n[5/12] Testing Simulation Flow (projectile-range)...')
...
print('\n[6/12] Testing Trust Boundary Enforcement (record_progress)...')
...
print('\n[7/12] Testing AI Socratic Hints with Gemini (get_hint)...')
...
print('\n[8/12] Testing 3D Space Visualization Generator (generate_3d_scene)...')
...
# TEST 9: NEW TOOL - 3D Math Surface Plotter (visualize_math_surface)
print('\n[9/12] Testing 3D Mathematical Surface Plotter (visualize_math_surface)...')
...
# TEST 10: NEW TOOL - Interactive Web Sandbox Preview (preview_html_css)
print('\n[10/12] Testing Interactive Web Sandbox (preview_html_css)...')
    ...
    json={
        ...
        'params': {
            ...
            'arguments': {
                ...
                'html': '<div class=\"card\"><h3>Interactive Card</h3><button onclick=\"alert(\'Clicked!\')\">Click Me</button></div>',
                'css': '.card { padding: 20px; border-radius: 12px; background: linear-gradient(135deg, #667eea, #764ba2); color: white; }',
                'javascript': 'console.log(\"Sandbox loaded\");'
            }
        }
    },
    headers=headers
...
assert_test('Sandbox CSS retained', 'linear-gradient' in sandbox_payload['css'])

# TEST 11: NEW TOOLS - Safe Python Scratchpad & Student Analytics
print('\n[11/12] Testing Python Scratchpad & Learning Analytics...')
# 11a: Scratchpad
scratch_resp = client.post(
    ...
    json={
        ...
        'id': 111,
        'method': 'tools/call',
        'params': {'name': 'run_code_scratchpad', 'arguments': {'code': 'numbers = [1, 2, 3, 4, 5]\nprint(\"SUM:\", sum(numbers))'}}
    },
    headers=headers
)
scratch_data = json.loads([l for l in scratch_resp.text.splitlines() if l.startswith('data:')][0][5:])
scratch_res = scratch_data['result']['structuredContent']
assert_test('Scratchpad executed successfully', scratch_res['success'] is True)
assert_test('Scratchpad captured stdout correctly', 'SUM: 15' in scratch_res['stdout'])
...
analytics_res = analytics_data['result']['structuredContent']
assert_test('Analytics student ID verified', analytics_res['studentId'] == fresh_student)
assert_test('Analytics total attempts tracked', analytics_res['totalAttempts'] >= 2)
assert_test('Analytics recommendation provided', bool(analytics_res['recommendation']))

# TEST 12: Public Cloudflare Tunnel Verification
print('\n[12/12] Testing Live Public Tunnel Endpoint...')
...
print('\n' + '=' * 70)
print('ALL 12 TEST SUITES PASSED! ALL 10 TOOLS & 3 WIDGETS ARE 100% OPERATIONAL!')
print('=' * 70)
